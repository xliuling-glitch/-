...
export default function Page() {
  ...
  const load = async (np=page) => { const r=await (await fetch(`/api/tasks?page=${np}&q=${encodeURIComponent(q)}&status=${status}&date=${date}`)).json(); setTasks(r.items||[]); setTotal(r.total||0); };
  useEffect(() => { load(); }, []);
  ...
  return <div className='space-y-3'>
    ...
    <FilterBar q={q} setQ={setQ} status={status} setStatus={setStatus} date={date} setDate={setDate} onSearch={()=>{setPage(1);load(1);}} />
    ...
    <div className='flex justify-end gap-2'><button className='border px-2' disabled={page<=1} onClick={()=>{const n=page-1;setPage(n);load(n);}}>上一页</button><span>{page} / {Math.max(1,Math.ceil(total/10))}</span><button className='border px-2' disabled={page>=Math.ceil(total/10)} onClick={()=>{const n=page+1;setPage(n);load(n);}}>下一页</button></div>
