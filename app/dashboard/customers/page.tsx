...
export default function Page() {
  ...
  const load = async (np=page,nq=q) => { const r=await (await fetch(`/api/customers?page=${np}&q=${encodeURIComponent(nq)}`)).json(); setList(r.items||[]); setTotal(r.total||0); };
  useEffect(() => { load(); }, []);
  ...
  return <div className='space-y-3'><h2 className='text-xl font-semibold'>客户管理</h2>
    <div className='bg-white border rounded p-3 flex gap-2'><input className='border p-2 rounded w-64' value={q} onChange={e=>setQ(e.target.value)} placeholder='搜索客户名称/编号'/><button className='bg-slate-800 text-white px-3 rounded' onClick={()=>{setPage(1);load(1,q)}}>搜索</button></div>
